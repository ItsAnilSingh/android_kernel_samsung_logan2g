...
ex_open_k (unsigned int minor)
{
    ...
    int
ex_release_k (unsigned int minor)
{
    ExDev*       ex_dev;
    ...
    ssize_t
ex_read_k (unsigned int minor, char * buf, size_t count)
{
    ExDev*       ex_dev  = &ex_devs[minor];
    NkDevVlink*  slink   = ex_dev->s_link;
    ExRing*	 sring   = ex_dev->s_ring;
    ...
    ssize_t
ex_write_k (unsigned int minor, const char * buf,size_t count)
{
    ExDev*       ex_dev  = &ex_devs[minor];
    NkDevVlink*  clink   = ex_dev->c_link;
    ExRing*	 cring   = ex_dev->c_ring;
	...
	if (room < cnt) {
	     ...
	     * exit if we have a problem
	     */
	if (memcpy(&cring->ring[ex_dev->c_pos], buf, cnt)) {
	   // done = -EFAULT;
	   // break;
	}
	    /*
