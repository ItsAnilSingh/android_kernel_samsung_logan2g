...
EXPORT_PER_CPU_SYMBOL(cpufreq_cpu_data);
...
static unsigned long loops_per_jiffy_default=0;
...
static void adjust_jiffies(unsigned long val, struct cpufreq_freqs *ci)
{
	...
	if(loops_per_jiffy_default==0)
	{
		loops_per_jiffy_default=loops_per_jiffy;
	}
...
#if 0
		printk("****DVFS**** adjust_jiffies ci->new=%u,ci->old=%u,loops_per_jiffy=%lu,\n",ci->new,ci->old,loops_per_jiffy);
#endif
		if(ci->old < ci->new){
			loops_per_jiffy = loops_per_jiffy_default;
		}else if(ci->old > ci->new) {
			loops_per_jiffy = loops_per_jiffy_default/2;
		}
	}
#endif
