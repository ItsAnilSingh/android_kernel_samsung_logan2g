...
static int32_t dsi_dpi_init(struct panel_spec* panel)
{
	...
	dpi_param.pixel_clock = 384*1000/11;
//	dpi_param.pixel_clock = 384*1000/9;
...
//	dsi_core_write_function(SPRD_MIPI_DSIC_BASE,  R_DSI_HOST_ERROR_MSK1, 0x3ffff);
 ...
 #ifndef CONFIG_FB_LCD_HX8369B_MIPI
...
int32_t sprdfb_dsi_ready(struct sprdfb_device *dev)
{
	...
	if(SPRDFB_MIPI_MODE_CMD == mipi->work_mode){
    	...
    	dsi_dpi_init(dev->panel); //thomaszhang@20130531 add
    	#endif
...
int32_t sprdfb_dsi_gen_write(uint8_t *param, uint16_t param_length)
{
	dsih_error_t result;
...
EXPORT_SYMBOL(sprdfb_dsi_gen_write);
...
static int32_t sprdfb_dsi_gen_read(uint8_t *param, uint16_t param_length, uint8_t bytes_to_read, uint8_t *read_buffer)
{
	...
	if(0 != (reg_val & 0x2)){
		printk("sprdfb: [%s] mipi read returns (0x%x)!\n", __FUNCTION__, reg_val);
		dsi_ctx.status = 2;
		result = 0;
	}

	if(0 != (reg_val_1 & 0x701)){
		printk("sprdfb: [%s] mipi read status returns!(0x%x, 0x%x)\n", __FUNCTION__, reg_val_1, reg_val_2);
		result = 0;
	}

	if(0 == result){
		printk(KERN_ERR "sprdfb: [%s] returns (%d)\n", __FUNCTION__, result);
		return -1;
	}
...
static int32_t sprdfb_dsi_dcs_read(uint8_t command, uint8_t bytes_to_read, uint8_t *read_buffer)
{
	...
	if(0 == result){
		printk(KERN_ERR "[%s] reg_val=0x%x, reg_val_1=0x%x, reg_val_2=0x%x\n", __FUNCTION__, reg_val, reg_val_1, reg_val_2 );
		return -1;
	}
...
{
	dsih_error_t result;
	result = mipi_dsih_gen_wr_packet(&(dsi_ctx.dsi_inst), 0, data_type,  p_params, param_length);
	return result;
}
...
static int32_t sprd_dsi_force_read(uint8_t command, uint8_t bytes_to_read, uint8_t * read_buffer)
{
	...
	result = mipi_dsih_gen_rd_packet(&(dsi_ctx.dsi_inst),  0,  6,  0, command,  bytes_to_read, read_buffer);
...
int32_t sprdfb_dsi_set_ulps(void)
{
    uint32_t reg_val;
    reg_val = dsi_core_read_function(SPRD_MIPI_DSIC_BASE, R_DSI_HOST_PHY_IF_CTRL);
    reg_val = reg_val | BIT(1) |BIT(3);
    dsi_core_write_function(SPRD_MIPI_DSIC_BASE, R_DSI_HOST_PHY_IF_CTRL,  reg_val);
    return 0;
}
